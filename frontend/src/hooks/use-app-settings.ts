...
export function useFeatureFlag(flagKey: string): boolean {
  const { enabled } = useFeatureFlagState(flagKey);
  return enabled;
}

/** enabled = flag on; isReady = settings loaded (do not treat "off" as real until isReady) */
export function useFeatureFlagState(flagKey: string): { enabled: boolean; isReady: boolean } {
  const { data: settings, isLoading } = useAppSettings();
  if (isLoading) {
    return { enabled: false, isReady: false };
  }
  const v = settings?.[flagKey];
  const enabled = v === true || v === 'true';
  return { enabled, isReady: true };
}
