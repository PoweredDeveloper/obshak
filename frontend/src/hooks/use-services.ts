...
export function useServiceCategories(queryOptions?: { enabled?: boolean }) {
  return useQuery({
    ...
    queryFn: async () => {
      return api<ServiceCategory[]>('/services/categories');
    },
...
export function useServices(options: UseServicesOptions = {}, queryOptions?: { enabled?: boolean }) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const page = await api<ApiPage<Service>>('/services', {
        query: {
          limit,
          offset,
...
interface ApiServiceReview extends Omit<ServiceReview, 'profiles'> {
  profile: { first_name: string | null; last_name: string | null; group_name: string | null } | null;
}
...
export function useServiceReviews(serviceId: string, options: { limit?: number; offset?: number } = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const page = await api<ApiPage<ApiServiceReview>>(`/services/${serviceId}/reviews`, {
        query: { limit, offset },
      });
      const reviews: ServiceReview[] = page.items.map((r) => ({
        id: r.id,
        ...
        profiles: r.profile
          ? {
              ...
              group_name: r.profile.group_name,
            }
          : undefined,
      }));
      return {
