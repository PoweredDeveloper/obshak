...
/** Week for parity: Sun 00:00 local → next Sat 24:00; flips at Sat→Sun midnight. */
export function startOfLocalSundayWeek(d: Date): Date {
...
export function getCurrentWeekType(semester: Semester): 'even' | 'odd' {
  ...
  const now = new Date();
  const anchorSunday = startOfLocalSundayWeek(start);
  const currentSunday = startOfLocalSundayWeek(now);
  const diffMs = currentSunday.getTime() - anchorSunday.getTime();
  const weekMs = 7 * 24 * 60 * 60 * 1000;
  const diffWeeks = Math.max(0, Math.floor(diffMs / weekMs));
