...
export default function AdminServicesPage() {
  ...
  const rawFlag = settings?.['features.services_enabled'];
  const servicesEnabled = rawFlag === true || rawFlag === 'true';
  ...
  const toggleServices = async (enabled: boolean) => {
    try {
      const { error } = await supabase.from('app_settings').upsert(
        {
          key: 'features.services_enabled',
          value: enabled,
          description: 'Показывать раздел «Услуги» в приложении',
