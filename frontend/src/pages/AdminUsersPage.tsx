...
import { Search, MessageCircle, RefreshCw, Shield, ShieldOff, ArrowUpDown } from 'lucide-react';
...
export default function AdminUsersPage() {
  ...
  const [bulkInstitute, setBulkInstitute] = useState('');
  const [bulkCourse, setBulkCourse] = useState('1');
  ...
  async function loadGroups() {
    const [profilesRes, groupsRes] = await Promise.all([
      supabase.from('profiles').select('group_name').not('group_name', 'is', null).order('group_name'),
      supabase.from('groups').select('name').order('name'),
    ]);

    const fromProfiles = [...new Set(profilesRes.data?.map(p => p.group_name).filter(Boolean) as string[])];
    const fromDb = (groupsRes.data || []).map((g) => g.name);
    const merged = [...new Set([...fromDb, ...fromProfiles])].sort((a, b) => a.localeCompare(b));
    setGroups(merged);
  }

  async function applyBulkGroupMetadata() {
    const groupName = bulkGroupName.trim();
    if (!groupName) {
      toast.error('Выберите группу');
      return;
    }
    const course = parseInt(bulkCourse, 10);
    const semester = parseInt(bulkSemester, 10);
    if (!Number.isFinite(course) || course < 1 || course > 10) {
      toast.error('Курс: от 1 до 10');
      return;
    }
    if (!Number.isFinite(semester) || semester < 1 || semester > 12) {
      toast.error('Семестр: от 1 до 12');
      return;
    }

    setSavingBulkGroup(true);
    const { data, error } = await supabase.rpc('admin_bulk_set_group_metadata', {
      p_group_name: groupName,
      p_institute: bulkInstitute.trim() || null,
      p_course: course,
      p_semester: semester,
      p_institute_id: null,
    });
    ...
    const result = data as { profiles_updated?: number; favorites_updated?: number } | null;
    toast.success(
      `Группа обновлена: ${result?.profiles_updated ?? 0} профилей, ${result?.favorites_updated ?? 0} в избранном`,
    );
    loadGroupStats();
        ...
        <Card className="p-4" id="users-list">
          ...
          <TabsContent value="groups" className="space-y-4 mt-4">
            <Card className="p-4 space-y-4">
              <h3 className="font-semibold">Группа целиком</h3>
              <p className="text-sm text-muted-foreground">
                Обновляет институт, курс и семестр у всех пользователей с этой группой и в избранном.
              </p>
              <Select value={bulkGroupName} onValueChange={setBulkGroupName}>
                <SelectTrigger>
              ...
              <Input
                ...
                value={bulkInstitute}
                onChange={(e) => setBulkInstitute(e.target.value)}
              />
              ...
              <Button onClick={applyBulkGroupMetadata} disabled={savingBulkGroup || !bulkGroupName}>
                {savingBulkGroup ? 'Сохранение...' : 'Применить ко всей группе'}
              </Button>
            </Card>
